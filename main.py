...
im_fname = sys.argv[1]
replace_type = im_fname.split('/')[-2]
root_fname   = '{}_blended.png'.format('_'.join(im_fname.split('.')[-2].split('/')[-1]))

os.makedirs(os.path.join(save_dir, replace_type), exist_ok=True)
...
def blend(im1_name, im2_name, mask_name):
        ...
        # Save image
        plt.imsave(os.path.join(save_dir, replace_type, root_fname), final)
        plt.imshow(final)
        plt.show()
